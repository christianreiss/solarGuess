...
def test_snow_loss_reduces_energy_and_marks_timeseries(monkeypatch):
    wx = SnowyWeather()
    clear_df = wx.df.copy()
    clear_df["snow_depth_cm"] = [0.0, 0.0, 0.0]
    ...
    result_clear = simulate_day(_scenario(), dt.date(2025, 1, 1), timestep="1h", weather_provider=wx)
    ...
    result_snow = simulate_day(_scenario(), dt.date(2025, 1, 1), timestep="1h", weather_provider=wx)
